...
namespace State {
    ...
    std::string EditingWWlModel::getElementAt(int i) {
        if (m_iType == LMODEL_PLANES) {
            ...
            std::string ret = m_hOwn->SprBank->GetAssetByIterator(m_hOwn->dbAssetsImg->getSelected())->GetIMGByIterator(
                    i)->GetName();
            return ret;
            ...
            if (ani->GetFrame(i)->GetKeyFrameName() != NULL)
                sprintf(tmp, "%s #%d (%s)", GETL2("WinDatabase", Lang_Db_Frame), i + 1,
                        ani->GetFrame(i)->GetKeyFrameName());
            else
            ...
            if (i == 0)
                return "-----";
            else if (m_hOwn->hSndBank->GetAssetByIterator(i - 1) != NULL)
            ...
            if (m_hOwn->hParser == NULL)
                return "";
            else if (m_hOwn->hSndBank->GetAssetByIterator(i) != NULL)
