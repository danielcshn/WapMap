...
int wmPlaceTile(lua_State *L) {
    ...
    if (x < 0 || y < 0 || x >= plane->GetPlaneWidth() || y >= plane->GetPlaneHeight()) {
        GV->Console->Printf("~r~Brush lua: invalid coords called in wmPlaceTile(%d, %d, %d) [valid range X:0-%d Y:0-%d",
                            x, y, tile,
    ...
    if (tile == BRUSH_OUTOFBOUND ||
        ...
        GV->editState->hTileset->GetSet(plane->GetImageSet(0))->GetTile(tile) == NULL && tile != BRUSH_EMPTY &&
        tile != BRUSH_FILLED) {
...
cBrush::cBrush(const char *pszName, cDataController *hDC, std::vector<std::string> vstrLayers, const char *ptr) {
    ...
    if (bSettingsFail) {
        GV->Console->Printf("~y~Warning ~w~calling (~y~%s ~w~[~y~%s~w~]): unable to call cbInitSettings", m_szName,
                            pszName);
    } else if (enset)
