...
bool cDataController::RenderImageRaw(byte *hData, HTEXTURE texDest, int iRx, int iRy, int iRowSpan, int w, int h,
                                     PID::Palette *pal) {
...
void cDataController::Think() {
    ...
    for (int f = 0; f < 3; f++) {
        for (size_t i = 0; i < vFiles[f].size(); i++) {
            ...
            if (ap) {
                std::string strnfp = vFiles[f].at(i).strPath.substr(
                        (ap->GetPath().length() > 0 ? ap->GetPath().length() + 1 : 0));
